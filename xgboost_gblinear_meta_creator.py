...
param_grid = [
              {'silent': [1], 'booster': ['gblinear'], 'nthread': [3], 'eval_metric': ['rmse'],
               'objective': ['reg:linear'], 'num_round': [3000], 'lambda': [0, 0.2, 0.4], 'alpha': [0, 0.2, 0.4],
               'subsample': [0.75], 'lambda_bias': [0, 0.2, 0.4]},
             ]
...
for i, params in enumerate(ParameterGrid(param_grid)):
    ...
    pd.DataFrame(meta_train).to_csv('meta_train_boost_gblinear_%d_v3.csv' % i)
    # train machine learning
    ...
    # predict
    predicted_results = xgclassifier.predict(xg_test)
    pd.DataFrame(predicted_results).to_csv('meta_test_boost_gblinear_%d_v3.csv' % i)
